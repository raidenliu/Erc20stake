    ...
    AccessControlUpgradeable
{
    ...
    struct Pool {
        ...
        // Weight of pool
        uint256 poolWeight;
        // Last block number that LTCs distribution occurs for pool
        uint256 lastRewardBlock;
        // Accumulated LTCs per staking token of pool
        uint256 accLTCPerST;
    ...
    struct User {
        ...
        uint256 stAmount;
        // Finished distributed LTCs to user
        uint256 finishedLTC;
        // Pending to claim LTCs
        uint256 pendingLTC;
    ...
    // ************************************** STATE VARIABLES **************************************
    // First block that LTCStake will start from
    uint256 public startBlock;
    // First block that LTCStake will end from
    uint256 public endBlock;
    // LTC token reward per block
    uint256 public LTCPerBlock;
    ...
    event SetLTC(IERC20 indexed LTC);
    ...
    event SetLTCPerBlock(uint256 indexed LTCPerBlock);
    ...
    event UpdatePool(uint256 indexed poolId, uint256 indexed lastRewardBlock, uint256 totalLTC);
    ...
    event Claim(address indexed user, uint256 indexed poolId, uint256 LTCReward);
    ...
    /**
     * @notice Set LTC token address. Set basic info when deploying.
     */
    function initialize(
        IERC20 _LTC,
        uint256 _startBlock,
        uint256 _endBlock,
        uint256 _LTCPerBlock
    ) public initializer {
        require(_startBlock <= _endBlock && _LTCPerBlock > 0, "invalid parameters");
    ...
    /**
     * @notice Set LTC token address. Can only be called by admin
     */
    function setLTC(IERC20 _LTC) public onlyRole(ADMIN_ROLE) {
        LTC = _LTC;
    ...
    /**
     * @notice Update the LTC reward amount per block. Can only be called by admin.
     */
    function setLTCPerBlock(uint256 _LTCPerBlock) public onlyRole(ADMIN_ROLE) {
        require(_LTCPerBlock > 0, "invalid parameter");

        LTCPerBlock = _LTCPerBlock;

        emit SetLTCPerBlock(_LTCPerBlock);
    }

    /**
     * @notice Add a new staking to pool. Can only be called by admin
     * DO NOT add the same staking token more than once. LTC rewards will be messed up if you do
     */
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns(uint256 multiplier) {
        ...
        bool success;
        (success, multiplier) = (_to - _from).tryMul(LTCPerBlock);
        require(success, "multiplier overflow");
    }

    /**
     * @notice Get pending LTC amount of user in pool
     */
    function pendingLTC(uint256 _pid, address _user) external checkPid(_pid) view returns(uint256) {
        return pendingLTCByBlockNumber(_pid, _user, block.number);
    }

    /**
     * @notice Get pending LTC amount of user by block number in pool
     */
    function pendingLTCByBlockNumber(uint256 _pid, address _user, uint256 _blockNumber) public checkPid(_pid) view returns(uint256) {
        Pool storage pool_ = pool[_pid];
        User storage user_ = user[_pid][_user];
        uint256 accLTCPerST = pool_.accLTCPerST;
        uint256 stSupply = pool_.stTokenAmount;

        if (_blockNumber > pool_.lastRewardBlock && stSupply != 0) {
            uint256 multiplier = getMultiplier(pool_.lastRewardBlock, _blockNumber);
            uint256 LTCForPool = multiplier * pool_.poolWeight / totalPoolWeight;
            accLTCPerST = accLTCPerST + LTCForPool * (1 ether) / stSupply;
        }

        return user_.stAmount * accLTCPerST / (1 ether) - user_.finishedLTC + user_.pendingLTC;
    }
    ...
    function updatePool(uint256 _pid) public checkPid(_pid) {
        ...
        (bool success1, uint256 totalLTC) = getMultiplier(pool_.lastRewardBlock, block.number).tryMul(pool_.poolWeight);
        require(success1, "totalLTC mul poolWeight overflow");

        (success1, totalLTC) = totalLTC.tryDiv(totalPoolWeight);
        require(success1, "totalLTC div totalPoolWeight overflow");

        uint256 stSupply = pool_.stTokenAmount;
        if (stSupply > 0) {
            (bool success2, uint256 totalLTC_) = totalLTC.tryMul(1 ether);
            require(success2, "totalLTC mul 1 ether overflow");

            (success2, totalLTC_) = totalLTC_.tryDiv(stSupply);
            require(success2, "totalLTC div stSupply overflow");

            (bool success3, uint256 accLTCPerST) = pool_.accLTCPerST.tryAdd(totalLTC_);
            require(success3, "pool accLTCPerST overflow");
            pool_.accLTCPerST = accLTCPerST;
        }

        pool_.lastRewardBlock = block.number;

        emit UpdatePool(_pid, pool_.lastRewardBlock, totalLTC);
    }
    ...
    /**
     * @notice Deposit staking BTC for LTC rewards
     */
    ...
    /**
     * @notice Deposit staking token for LTC rewards
    ...
    function unstake(uint256 _pid, uint256 _amount) public whenNotPaused() checkPid(_pid) whenNotWithdrawPaused() {
        ...
        uint256 pendingLTC_ = user_.stAmount * pool_.accLTCPerST / (1 ether) - user_.finishedLTC;

        if(pendingLTC_ > 0) {
            user_.pendingLTC = user_.pendingLTC + pendingLTC_;
        }
        ...
        user_.finishedLTC = user_.stAmount * pool_.accLTCPerST / (1 ether);
    ...
    /**
     * @notice Claim LTC tokens reward
     *
    ...
    function claim(uint256 _pid) public whenNotPaused() checkPid(_pid) whenNotClaimPaused() {
        ...
        uint256 pendingLTC_ = user_.stAmount * pool_.accLTCPerST / (1 ether) - user_.finishedLTC + user_.pendingLTC;

        if(pendingLTC_ > 0) {
            user_.pendingLTC = 0;
            _safeLTCTransfer(msg.sender, pendingLTC_);
        }

        user_.finishedLTC = user_.stAmount * pool_.accLTCPerST / (1 ether);

        emit Claim(msg.sender, _pid, pendingLTC_);
    }
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (user_.stAmount > 0) {
            // uint256 accST = user_.stAmount.mulDiv(pool_.accLTCPerST, 1 ether);
            (bool success1, uint256 accST) = user_.stAmount.tryMul(pool_.accLTCPerST);
            require(success1, "user stAmount mul accLTCPerST overflow");
            (success1, accST) = accST.tryDiv(1 ether);
            require(success1, "accST div 1 ether overflow");
            
            (bool success2, uint256 pendingLTC_) = accST.trySub(user_.finishedLTC);
            require(success2, "accST sub finishedLTC overflow");

            if(pendingLTC_ > 0) {
                (bool success3, uint256 _pendingLTC) = user_.pendingLTC.tryAdd(pendingLTC_);
                require(success3, "user pendingLTC overflow");
                user_.pendingLTC = _pendingLTC;
            }
        }
        ...
        // user_.finishedLTC = user_.stAmount.mulDiv(pool_.accLTCPerST, 1 ether);
        (bool success6, uint256 finishedLTC) = user_.stAmount.tryMul(pool_.accLTCPerST);
        require(success6, "user stAmount mul accLTCPerST overflow");

        (success6, finishedLTC) = finishedLTC.tryDiv(1 ether);
        require(success6, "finishedLTC div 1 ether overflow");

        user_.finishedLTC = finishedLTC;

        emit Deposit(msg.sender, _pid, _amount);
    }

    /**
     * @notice Safe LTC transfer function, just in case if rounding error causes pool to not have enough LTCs
     *
     * @param _to        Address to get transferred LTCs
     * @param _amount    Amount of LTC to be transferred
     */
    function _safeLTCTransfer(address _to, uint256 _amount) internal {
        uint256 LTCBal = LTC.balanceOf(address(this));

        if (_amount > LTCBal) {
            LTC.transfer(_to, LTCBal);
        } else {
            LTC.transfer(_to, _amount);
        }
    }
